    ...
    <div className="bg-white dark:bg-slate-800 rounded-2xl p-6 mb-6 shadow-sm">
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {rows.map((r) => (
              <tr
                key={r.ticker}
                className={`border-b border-slate-100 dark:border-slate-700/50 ${r.shares === 0 ? "opacity-50" : ""}`}
              >
                <td className="py-2 pr-4 text-slate-800 dark:text-slate-200 font-medium">
                  {r.shares === 0 && <span className="text-red-500 text-xs mr-1">✗</span>}
                  {r.name}
                </td>
                <td className="py-2 pr-4 hidden md:table-cell">
                  <span className="font-mono text-blue-500 text-xs bg-blue-50 dark:bg-blue-950/50 px-2 py-0.5 rounded">{r.ticker}</span>
                </td>
                <td className="py-2 pr-4 text-right tabular-nums text-slate-600 dark:text-slate-300">{(r.weight * 100).toFixed(0)}%</td>
                ...
                <td className="py-2 text-right tabular-nums hidden sm:table-cell">
                  {r.shares > 0 ? (
                    <span className={Math.abs(r.weightDiff) > 0.02 ? "text-yellow-500" : "text-slate-400"}>
                      {r.weightDiff >= 0 ? "+" : ""}{(r.weightDiff * 100).toFixed(1)}%
                    </span>
                  ) : <span className="text-red-500 text-xs">insuffisant</span>}
                </td>
              </tr>
            ))}
          </tbody>
      ...
      <div className="mt-5 grid grid-cols-2 md:grid-cols-4 gap-3">
        <div className="bg-slate-50 dark:bg-slate-700 rounded-xl p-3">
          <p className="text-slate-500 dark:text-slate-400 text-xs mb-1">Total investi</p>
          <p className="text-slate-800 dark:text-slate-100 font-bold">{fmt(totalInvested)} $</p>
        </div>
        <div className="bg-slate-50 dark:bg-slate-700 rounded-xl p-3">
          <p className="text-slate-500 dark:text-slate-400 text-xs mb-1">Cash résiduel</p>
          <p className={`font-bold ${cashLeft > 0 ? "text-slate-800 dark:text-slate-100" : "text-red-500"}`}>{fmt(cashLeft)} $</p>
        </div>
        <div className="bg-slate-50 dark:bg-slate-700 rounded-xl p-3">
          <p className="text-slate-500 dark:text-slate-400 text-xs mb-1">% du budget investi</p>
          <p className="text-slate-800 dark:text-slate-100 font-bold">{budget > 0 ? ((totalInvested / budget) * 100).toFixed(1) : "0"}%</p>
        </div>
        <div className="bg-slate-50 dark:bg-slate-700 rounded-xl p-3">
          <p className="text-slate-500 dark:text-slate-400 text-xs mb-1">Titres sans position</p>
