...
export function AboutModal({ name, description, t0, count, onClose }: Props) {
  const t0Formatted = t0
    ? new Date(t0 + "T00:00:00").toLocaleDateString("fr-CA", { dateStyle: "long" })
    : "—";
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white dark:bg-slate-800 rounded-2xl p-8 max-w-2xl w-full max-h-[85vh] overflow-y-auto shadow-2xl border border-slate-200 dark:border-slate-700">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-slate-800 dark:text-slate-100 font-bold text-xl">À propos de {name}</h2>
        ...
        <div className="space-y-5 text-sm text-slate-600 dark:text-slate-300">
          <section>
            <h3 className="text-slate-800 dark:text-slate-100 font-semibold mb-2">Qu'est-ce que {name} ?</h3>
            <p>{description}</p>
            {count > 0 && (
              <p className="mt-2 text-slate-500 dark:text-slate-400">Composition actuelle : <span className="text-slate-700 dark:text-slate-300">{count} titres</span>. Calcul selon la méthode du diviseur, standard des grands indices mondiaux.</p>
            )}
          </section>
          ...
          <section>
            ...
            <p>L'indice a été initialisé le <span className="text-slate-800 dark:text-slate-100">{t0Formatted}</span> à une valeur de base de <span className="text-slate-800 dark:text-slate-100">1 000 points</span>.</p>
          </section>
          ...
          <section className="border-t border-slate-200 dark:border-slate-700 pt-4">
            <p className="text-slate-400 dark:text-slate-500 text-xs">⚠ {name} est un indice expérimental à titre éducatif uniquement. Il ne constitue pas un conseil de placement. Les performances passées ne garantissent pas les résultats futurs.</p>
          </section>
        </div>
      </div>
    </div>
